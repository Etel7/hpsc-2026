...
__global__ void kernel_optimized_v13(int dim_m, int dim_n, int dim_k,
    ...
    // Double buffering sur la dimension d'étape [2]
    ...
    int write_stage = 0;
    {
        ...
        for (int step = 0; step < 4; ++step) {
            ...
            if (global_m < dim_m && row_k < dim_k) {
                float4 tmp = reinterpret_cast<const float4*>(&d_a[row_k * dim_m + global_m])[0];
        ...
        for (int step = 0; step < 4; ++step) {
            int total_f4 = threadIdx.x + step * 256; 
            int row_k = (total_f4 % 8) * 4;          
            int col_n = total_f4 / 8;                
            int global_n = offset_b_n + col_n;
            if (global_n < dim_n && row_k < dim_k) {
                float4 tmp = reinterpret_cast<const float4*>(&d_b[global_n * dim_k + row_k])[0];
    ...
    // --- BOUCLE PRINCIPALE PIPELINÉE ---
    for (int k = 32; k < dim_k; k += 32) {
        int read_stage = write_stage;
        write_stage = 1 - write_stage;

        // 1. Lancement asynchrone / anticipé du chargement du PROCHAIN bloc (k) pendant que l'actuel calcule
        #pragma unroll
        for (int step = 0; step < 4; ++step) {
            int total_f4 = threadIdx.x + step * 256; 
            int row_k = total_f4 / 32;               
        ...
        // 2. CALCULS SUR LE STAGE ACTUEL (Pendant que la mémoire globale voyage vers le second stage)
        #pragma unroll
        ...
        // On synchronise avant de switcher les buffers au prochain tour
        __syncthreads();
    }

    // --- DERNIER TOUR DE CALCULS (Épuisement du pipeline) ---
    int read_stage = write_stage;
    #pragma unroll
    for (int ki = 0; ki < 2; ki++) {
        int k_offset = ki * 16;
...
int main(int argc, const char **argv) {
    ...
    for (int i = 0; i < Nt+2; i++) {
        if (i == 2) tic = chrono::steady_clock::now();
        kernel_optimized_v13<<< grid, block >>>(m, n, k, A, B, C2);
        cudaDeviceSynchronize();
    }
