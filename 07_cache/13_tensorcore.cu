...
__global__ void kernel_optimized_v11(int dim_m, int dim_n, int dim_k,
    ...
    // block_b change de forme pour éliminer les Bank Conflicts à l'écriture
    __shared__ half block_a[2][16][TILE_SIZE + 8];
    __shared__ half block_b[2][TILE_SIZE][16 + 8]; 
    ...
    for (int k = 0; k < dim_k + 16; k += 16) {
        ...
        if (k < dim_k) {
            ...
            for (int step = 0; step < 2; ++step) {
                ...
                if (global_n < dim_n && global_k < dim_k) {
                    ...
                    // Écriture optimisée sans aucun Bank Conflict
        ...
        // 2. CALCULS TENSOR CORES OPTIMISÉS
        if (k > 0) {
            int read_stage = 1 - write_stage;

            // OPTIMISATION : On charge Matrix B UNE SEULE FOIS à l'extérieur du sous-bloc
            wmma::fragment<wmma::matrix_b, 16, 16, 16, half, wmma::col_major> b_frag[2];
            #pragma unroll
            for (int c = 0; c < 2; c++) {
                int col_tile = warp_n * 2 + c;
                wmma::load_matrix_sync(b_frag[c], &block_b[read_stage][col_tile * 16][0], 16 + 8);
            }
...
int main(int argc, const char **argv) {
    ...
    // Retour aux穩 stables 256 threads
    dim3 block = dim3(256);
    dim3 grid = dim3((m + TILE_SIZE - 1) / TILE_SIZE, (n + TILE_SIZE - 1) / TILE_SIZE);

    for (int i = 0; i < Nt+2; i++) {
        if (i == 2) tic = chrono::steady_clock::now();
        kernel_optimized_v11<<< grid, block >>>(m, n, k, A, B, C2);
        cudaDeviceSynchronize();
    }
